...
export function initializeTable(blockIdCounter, rows = 3, cols = 3, hasHeader = true, hasFooter = false) {
    const cells = [];
...
export function addTableRow(blocks, blockId, blockIdCounter, position = 'bottom') {
...
export function removeTableRow(blocks, blockId, rowIndex) {
    const block = blocks.find(b => b.id === blockId);
...
export function addTableColumn(blocks, blockId, blockIdCounter, position = 'right') {
    const block = blocks.find(b => b.id === blockId);
    if (!block || block.type !== 'table' || !block.tableData) return null;
...
export function removeTableColumn(blocks, blockId, colIndex) {
    const block = blocks.find(b => b.id === blockId);
    if (!block || block.type !== 'table' || !block.tableData) return;
...
export function mergeTableCells(blocks, blockId, startRow, startCol, endRow, endCol) {
    const block = blocks.find(b => b.id === blockId);
    if (!block || block.type !== 'table' || !block.tableData) return;
    ...
    if (startCol > endCol) [startRow, endCol] = [endCol, startCol];
...
// Unmerge cells
export function unmergeTableCells(blocks, blockId, row, col) {
    const block = blocks.find(b => b.id === blockId);
    if (!block || block.type !== 'table' || !block.tableData) return;
...
// Update cell content
export function updateTableCellContent(blocks, blockId, cellId, content) {
    const block = blocks.find(b => b.id === blockId);
    if (!block || block.type !== 'table' || !block.tableData) return;
...
// Toggle header
export function toggleTableHeader(blocks, blockId) {
    const block = blocks.find(b => b.id === blockId);
    if (!block || block.type !== 'table') return;
    
    if (!block.tableData) {
        block.tableData = initializeTable(0, 3, 3, true, false);
    }
...
// Toggle footer
export function toggleTableFooter(blocks, blockId) {
    const block = blocks.find(b => b.id === blockId);
    if (!block || block.type !== 'table') return;
    
    if (!block.tableData) {
        block.tableData = initializeTable(0, 3, 3, false, true);
    }
